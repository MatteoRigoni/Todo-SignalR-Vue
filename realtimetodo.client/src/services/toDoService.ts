...
export default class ToDoService {
  ...
  constructor() {
    ...
    this.connection = new HubConnectionBuilder()
      ...
      this.connection.on("updateToDoList", (values: any[]) => {
        this.events.emit("updatedToDoList", values);
      });

      this.connection.on("updatedListData", (values: any[]) => {
  ...
  install(Vue: VueConstructor<Vue>, pluginOptions: any) {
    Vue.prototype.$connection = this.connection;
...
export const ConnectionServices: PluginObject<any> = {
  install(Vue: VueConstructor<Vue>, options: any | undefined) {
